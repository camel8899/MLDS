...
CHECK_PATH = './save/S2VT_EnDe/256-clip25_drop3_lr-3_3000_attn/{}.tar'.format(sys.argv[1])
checkpoint = torch.load(CHECK_PATH)
E = Encoder(4096,hidden_size)
E.load_state_dict(checkpoint['encoder'])
D = Decoder(3*hidden_size,hidden_size)
...
with open('output_attn_{}.csv'.format(sys.argv[1]),'w') as f:
	for data in DS:
		caption_id = data[1]
		data = data[0]
		...
		decoder_padding = Variable(torch.zeros(1,feat.size()[1],2*hidden_size).cuda())
		...
		encoder_padding = Variable(torch.zeros(1,50,4096).cuda())
		encoder_output2, encoder_hidden = E(encoder_padding,encoder_hidden)
		result = []
		for i in range(MAX_LENGTH):
			attn_energies = Variable(torch.zeros(1, 80).cuda()) # B x S
			for b in range(1):
				for l in range(80):
					attn_energies[b,l] = torch.dot(decoder_hidden[0][0,b],encoder_output1[b,l])
			attn =	F.softmax(attn_energies).view(1,1,-1)
